...
export default function Home() {
  ...
  const fetchUserBoards = async (userId: string) => {
    ...
    try {
      ...
      const formattedSharedBoards = sharedWithUser
        .map((item) => {
          // Ensure item.boards is treated as a single object and exists
          const boardData = Array.isArray(item.boards) ? item.boards[0] : item.boards
          if (!boardData) return null // Skip if board data is missing
        ...
        .filter(board => board !== null) // Filter out any null entries
      ...
      } else if (formattedSharedBoards.length > 0 && formattedSharedBoards[0]) {
